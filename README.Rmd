...
## Top 10 Daily Excess Returns as of `r Sys.Date()`
Updated for the available data range from `r earliest_date` to 
`r most_recent_date`. Calculated as the geometric mean of daily excess returns.
Included for bragging rights :).

``` {r echo = FALSE, message = FALSE}
participating_student_reports %>%
  dplyr::filter(!is.infinite(excess_gmrr)) %>%
  ...
  kableExtra::kable(
    caption = paste0("Top 10 Excess Returns ", Sys.Date()), 
    escape  = FALSE,
    align   = rep('c', 5)
  )
```
