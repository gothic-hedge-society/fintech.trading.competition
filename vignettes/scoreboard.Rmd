...
shiny::tagList(
  ...
  shiny::fluidRow(
    ...
    shiny::column(
      width = 4, 
      shiny::a(
        shiny::img(src = "horizen_logo.png"), 
...
sharpe_traders <- sharpes[nrow(sharpes),-1] %>% 
  unlist() %>%
  sort(decreasing = TRUE) %>% 
  head(n_traders) %>%
  names() %>%
  c(
    # Always include these traders
    "hoppity", "Trader Raven", "bubbles", "Kalamazee", "Samantha",
    "Nonfortissimus", "wenjiez4", "jlau", "Lionelooooo", "z10g" 
  ) %>%
  intersect(
...
sharpes %>%
  dplyr::select(Date, dplyr::all_of(sharpe_traders)) %>% {
